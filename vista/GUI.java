...
public class GUI {
	...
	private static final int tamanioAviso = 9;//18;
	
	private static final int tamanioTexto = 9;//17;
	...
	private void initialize() {
		...
		//FUENTE GLOBAL
		setUIFont (new javax.swing.plaf.FontUIResource(new Font("ARIAL",Font.PLAIN, tamanioTexto)));
		//Fuente para avisos de colores, de pendientes, retrasadas y devolver
		Font avisos = new Font("Arial", Font.PLAIN, tamanioAviso);
		...
		tabbedPane.addTab("PEL�CULAS", iconMovie, pBusqueda, null);
		...
		panel_24.setBorder(new TitledBorder(null, "Actualizar/Alta de pel�culas", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		textoPelicula.setText("Escribe una pel�cula");
		...
		panelAlquilarDevolver.setBorder(new TitledBorder(null, "Alquilar o devolver pel�culas, pendientes y retrasos", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		JDatePanelImpl datePanel = new JDatePanelImpl(modeloFecha, p);
		...
		bAlquilar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseReleased(MouseEvent arg0) {
				System.out.println("Alquilando pel�cula "+pCod.getText());
		...
		txtpnAlquilamos.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent arg0) {
				if (arg0.getKeyChar() == KeyEvent.VK_SPACE) {
					System.out.println("Devuelve pel�cula dando al ESPACIO");
		...
		txtpnAlquilamos.setText("�Desea devolver la pel�cula? (Pulse espacio para confirmar)");
		...
		comboSocios.setModel(new DefaultComboBoxModel<String>(new String[] {"Click aqu� para SOCIO"}));
		...
		comboHaVisto.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				System.out.println("[COMBO] busca EN LA BASE DE DATOS si el socio: "+sSoc.getText()+" alquil� la peli: "+textHaVisto.getText());
		...
		textHaVisto.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent arg0) {
				if (arg0.getKeyChar() == KeyEvent.VK_ENTER) {
					System.out.println("[ENTER] busca EN LA BASE DE DATOS si el socio: "+sSoc.getText()+" alquil� la peli:"+textHaVisto.getText());
					controlador.cuandoHaVistoElSocioLaPeliculaDB(textHaVisto.getText(), sSoc.getText());
				}
...
////////////////////////////////////////
		...
		//Artima�a sucia para ganar el foco en los jtextfield de socio y pelicula cuando cambia de pesta�a
	...
	listaRestrasos.setBorder(new TitledBorder(null, "Listado de socios que deben pel�culas", TitledBorder.LEADING, TitledBorder.TOP, null, null));
	...
	listaPeliculas.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Listado de pel�culas ordenadas por t�tulo, pulsa en las columnas para reordenar", TitledBorder.LEADING, TitledBorder.TOP, null, null));
...
//			A�adido por versi�n 2.4.0 avisar de si la ha visto salvado el pisto de los t�tulos					
			System.out.println("[updateVistaSocio] parecidas para el socio: ["+sSoc.getText()+ "] de la pel�cula ["+pCod.getText()+"]");
			...
			//Si no esta vac�o el campo observaciones se pone naranja
			...
			//Pendiente de revisi�n, es para "ha visto peli" que esta deprecado encubierto por haVistoParecidas
...
//			Cambio v2.4.2
			...
			//De lo contrario el campo Titulo no se sabr� (busca pelicula tiene l�gica
			...
			JOptionPane.showMessageDialog(this.getFrmVideoClubCapitn(), "No se encuentran pel�culas con esos datos.");

	}
	

	/**
	 * La pel�cula est� disponible, vamos a alquilar
	 */
	public void logicaBotonAlquilar()
	{
		...
		//si est� disponible y hay codigo de socio relleno y c�digo de pel�cula relleno
		...
		//Hay codigo de pelicula relleno pero a�n falta el c�digo del socio
		...
		//si no hay n�mero de pel�cula o no hay n�mero de socio
	...
	/**
	 * La pel�cula est� alquilada, vamos a devolverla
	 */
	...
	public void listaDeudores(Boolean quick)
	{
		...
		modeloTablaDeudores.setColumnIdentifiers(new String[] {"Tarjeta", "Nombre","C�digo","T�tulo", "Fecha", "Retrasos"});
	...
	public void listaAlquileres(Boolean quick)
	{
		...
		modeloTablaAlquileres.setColumnIdentifiers(new String[] {"Tarjeta", "Nombre","C�digo","T�tulo", "Fecha"});
	...
	private void listaSocios()
	{
		...
		modeloTablaSocios.setColumnIdentifiers(new String[] {"Tarjeta", "Nombre", "N�mero de alquileres"});
	...
	private void listaPeliculas()
	{
		...
		modeloTablaPeliculas.setColumnIdentifiers(new String[] {"C�digo", "T�tulo", "Fecha de alta", "Precio"});
	...
	public void cartelDeudas(int idSocio, int retrasos, boolean visible) {
		...
		if (visible) {
			text = "El socio " + idSocio + " debe " + retrasos + " pel�cula(s)";
	 ...
	 * se han pasado el d�a de la devoluci�n
	 * @param idSocio
	 * @param pendientes
	 * @param visible
	 */
	...
	public void cartelRetrasos(int p_idSocio, String valor, int retrasos, boolean visible) {
		...
		{
			this.txtpnAlquilamos.setText("�Desea devolver la pel�cula? (Pulse espacio para confirmar)");
	...
	public void setTablaPendientes(ArrayList<String[]> tupla) {
		int id = 0;
		modeloTablaPendientes.setRowCount(0);
		modeloTablaPendientes.setColumnIdentifiers(new String[] {"C�digo","T�tulo", "Fecha"});
		for (String[] t : tupla){
			//�Apa estelar
	...
	public void setTablaDeudas(ArrayList<String[]> tupla) {
		int id = 0;
		modeloTablaDeudas.setRowCount(0);
		modeloTablaDeudas.setColumnIdentifiers(new String[] {"C�digo","T�tulo", "Fecha", "Retrasos"});
		for (String[] t : tupla){
			//�Apa estelar
			//Edito a fuego la columna de los ids de la peli para no perder tiempo con 3 left joints
			id = controlador.getIDPelicula(t[3]);
