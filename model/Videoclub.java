...
public class Videoclub {
	...
	public Videoclub() {
		...
		//Se hace 3 veces (aqu� y al actualizar pelis y socios), es importante para 
		...
		//Si es un nuevo d�a empieza en cero, si no coge lo que haya en el fichero
	...
	/**
	 * Hay que ampliar bastante �ste m�todo, quieren que saque retrasos
	 * @param columna
	 * @param valor
	 */
	...
	/**
	 * De lo �ltimo que hice, para sacar los 3 �ltimos socios que han visto la pel�cula
	 * que entra (a devolver o a alquilar) y estudiar al �ltimo socio (pendientes y retrasos)
	 * @param columna
	 * @param valor
	 */
	public void get3SociosPeliculaPendientesDeudasRetrasos(String columna, String valor) {
		...
		if (tupla.size()>0)
		{
			//Busca para la pel�cula los 3 �ltimos socios que la han visto y estudia al primero: 
			...
			//A�adido en 2.4.2 para avisar de que el que viene a devolver tiene alg�n aviso
	...
	private ArrayList<String[]> compruebaRetrasos(String titulo, int p_idSocio, Boolean quick) 
	{
		...
		for (int row = 0; row < numeroDeudores; row++)
		{
			...
			if (p_idSocio ==  (int) this.vista.getModeloTablaRetrasados().getValueAt(row, 0)) 
			{
				...
				//Reservo memoria para guardar las columnas + 1 para colocar ah� los retrasos 
				//Adem�s le digo que recorra uno menos del modelo, porque hay 5 columnas y no 6
				//VAYA TRUCO!!!
	...
	/**
	 * Obtiene el n�mero de retrasos que tiene una pel�cula, 
	 * se resta uno porque se devuelve al d�a siguiente 
	 * y ese d�a no se cuenta como retraso
	 * @param fecha es la fecha en que se alquil� la pel�cula
	 * @return retrasos - 1
	 */
	...
	{
		//Tratamiento y protecci�n de campos
		int numPases = 0;
		int numPVP = 0;
		int numRetrasos = 0;
		...
		//Actualizamos TIERS y seteamos en la vista para tener las b�squedas frescas
		this.actualizaTiers();
		vista.settPeliculas(tPeliculas);
			...
			String direccion, String telefono, String numAlqui, String notas, Boolean avisar, String email)
	{
		...
		try {
			...
			System.out.println("[EXCEPTION] public void updateSocio( N�MERO DE ALQUILERES NO ES UN N�MERO )");
		}
	...
	public void altaPelicula()
	{
		...
		if (id == -1)
			...
			JOptionPane.showMessageDialog(this.vista.getFrmVideoClubCapitn(), "Alta correcta, ahora rellena la ficha o d�jalo para despu�s.");
	 ...
	 * puede ocurrir que le de de plazo hasta una hora avanzada del d�a del retraso
	 ...
	 * Pero de la forma en que lo hago (sumando 2 o 3 d�as), si considero la hora, 
	 * tendr�a hasta el mi�rcoles a las 18.39 para devolver, lo cual no es correcto.
	 * @return
	 */
	...
	public void cuandoHaVistoElSocioLaPeliculaDB(String titulo, String tarjeta) {
		...
		if (aux == null)
			JOptionPane.showMessageDialog(this.vista.getFrmVideoClubCapitn(), "El socio "+tarjeta+", no ha visto la pel�cula "+titulo+".");
		else
			JOptionPane.showMessageDialog(this.vista.getFrmVideoClubCapitn(), "El socio "+tarjeta+", ha visto la pel�cula "+titulo+" en "+aux.toString()+".");
		
	}
	

	/**
	 * M�todo m�s M�GICO Y DELICADO
	 * 
	 * Partiendo de la info extra�da de la BD se confecciona una lista de socios
	 * con deudas y el d�a que alquilaron y falta a�adir los d�as de retraso
	 */
...
//		System.out.println("	E L A P S E D getHistoricosDBPerez-> " + (double)diff_time / 1000000000.0);
		...
		for (String tuplaAlq : alquiladas)
		{
			...
			//Recorro el hist�rico de alquileres por ordenado por fecha desc (order by fecha desc)
			//Alquiler m�s reciente arriba del todo! (pero hay que ver que siga alquilada (siguiente if)
			for (String[] tuplaHist : historico)
			{			
				//If pelicula alquilada est� en el hist�rico... 
				//procesamos y pasamos al siguiente t�tulo con alquilado = true  (hay un break al final de �ste if)
				//para que no siga buscando una vez que encuentra el t�tulo alquilado m�s reciente en el 
				//hist�rico.
				if (tuplaAlq.equals(tuplaHist[3]))
				{
					...
					if (Boolean.valueOf(tuplaHist[5]))
					{
						try {
							...
							// La fecha ser� hasta el martes 00:00 para que tenga todo el 
					...
					else if (isSabado(tuplaHist[4]))
					{
						try {
							...
							// la fecha ser� el martes que sea a las 00:00 para dejarle devolver todo el lunes hasta las 23:59
						...
						if (fechaDevolucion.before(dameHoy()))
						{
							...
							listaBuenaGente.add(tuplaHist);
					}//fin else if es S�bado
					
					
					//Un d�a normal entiendo yo... puede que se escapen casos
					else
					{
						try {
							c.setTime(sdf.parse(tuplaHist[4]));
					...
					//Acierto, pues rompo el bucle y paso a la siguiente pel�cula alquilada
					break;
				}//fin del if del acierto
			}//fin del for del hist�rico
		}//fin del for de alquiladas = true
		
		//Debugging... algun d�a lo borrar�
...
//		}
		...
		try {
			...
			while ((text = reader.readLine()) != null) {
				try {
					...
					this.vista.updateAlquiladasHoy(this.alquiladasHoy);
					System.out.println("[EXCEPTION] Fichero contiene un valor no num�rico:["+ text +"]");
				}
			}
		} catch (FileNotFoundException e) {
			//e.printStackTrace();
			System.out.println("Fichero no encontrado para recuperar alquileres, al cerrar se crear� con fecha: "+filename);
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			try {
	...
	/**
	 * Se llama desde la pesta�a socios o pel�culas al buscar un socio. 
	 * Sirve para pintar las dos tablas
	 * en la pesta�a de los socios
	 * @param tarjeta
	 */
	public void buscaPendientesRetrasosTablaPestaniaSocio(String tarjeta) 
	{
		...
		//null, que es el t�tulo de la pel�cula para mirar los retrasos de la pel�cula,
		//Desde la pesta�a socio no se que pel�cula se est� alquilando (podr�a pero no lo han pedido)
	...
	/**
	 * Inclu�do en la  versi�n 2.4.1 con la expresi�n regular 
	 * @param idPeli
	 * @param idSocio
	 * @return
	 */
